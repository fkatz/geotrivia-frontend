...
class POIRepository {
  ...
  async postOne(poi: POI): Promise<POI | null> {
    try {
      let response = await this.fetchJSON("/api/pois/", "POST", poi);
      if ('id' in response) {
        return response;
  ...
  async putOne(poi: POI): Promise<POI | null> {
    try {
      let response = await this.fetchJSON("/api/pois/" + poi.id, "PUT", poi);
      if ('id' in response) {
        return response;
      } else throw new Error("Invalid response");
    } catch (e) {
